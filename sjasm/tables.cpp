...
void CLabelTable::Dump() {
	...
	for (const auto& symbol : symbols) {
		if (LABEL_PAGE_UNDEFINED == symbol.second.page) continue;
		ep = line;
		*(ep) = 0;
		*(ep++) = '0';
		*(ep++) = 'x';
		PrintHexAlt(ep, symbol.second.value);
		*(ep++) = ' ';
		*(ep++) = symbol.second.used ? ' ' : 'X';
		*(ep++) = ' ';
		STRNCPY(ep, LINEMAX, symbol.first.c_str(), LINEMAX - (ep - line) - 2);
		STRNCAT(ep, LINEMAX, "\n", 2);
		fputs(line, listFile);
	}
}
...
void CLabelTable::DumpForUnreal() {
	...
	for (const auto& symbol : symbols) {
		...
		int page = Options::EmitVirtualLabels ? LABEL_PAGE_OUT_OF_BOUNDS : symbol.second.page;
		...
		int lvalue = symbol.second.value & ADR_MASK;
		...
		*(ep++) = ' ';
		STRCPY(ep, LINEMAX-(ep-ln), symbol.first.c_str());
		STRCAT(ep, LINEMAX, "\n");
...
void CLabelTable::DumpForCSpect() {
	...
	for (const auto& symbol : symbols) {
		if (LABEL_PAGE_UNDEFINED == symbol.second.page) continue;
		const int labelType =
			symbol.second.isStructEmit ? 0 :
			symbol.second.isStructDefinition ? 4 :
			symbol.second.IsEQU ? 1 :
			symbol.second.IsDEFL ? 2 :
			(LABEL_PAGE_ROM <= symbol.second.page) ? 3 : 0;
		const short page = labelType ? 0 : symbol.second.page;
			// TODO:
		...
		const aint longAddress = (PAGE_MASK & symbol.second.value) + page * PAGE_SIZE;
		fprintf(file, "%08X %08X %02X ", 0xFFFF & symbol.second.value, longAddress, labelType);
		// convert primary+local label to be "@" delimited (not "." delimited)
		STRCPY(temp, LINEMAX, symbol.first.c_str());
...
void CLabelTable::DumpSymbols() {
	...
	for (const auto& symbol : symbols) {
		if (!isalpha((byte)symbol.first[0])) continue;
		WriteLabelEquValue(symbol.first.c_str(), symbol.second.value, symfp);
	}
	fclose(symfp);
}
